...
# =========================
# 🔥 STATUS (agora correto)
# =========================
df["Status_Programacao"] = df["Programação"].apply(
    lambda x: "Hoje" if pd.notnull(x) and pd.to_datetime(x).date() == hoje
    else ("Realizado" if pd.notnull(x) and pd.to_datetime(x).date() < hoje else None)
)
